...
from slam_robot.utils.constants import minimum_distance_between_clusters, minimum_points_in_cluster


class Cluster:
    TOLERANCE_FOR_CIRCLE_COHERENCE = 12
    FIX_BEACON_RADIUS = 25
    """
    Cluster of points. It may be an obstacle or a beacon.
    """
    ...
    #
    #     beacon = None
    #     if np.isclose(solution.fun[0], 0, atol=self.TOLERANCE_FOR_CIRCLE_COHERENCE):
    #         beacon = CylinderBeacon()
    #         beacon.set_parameters(solution.x[0], solution.x[1], self.FIX_BEACON_RADIUS, 0)
